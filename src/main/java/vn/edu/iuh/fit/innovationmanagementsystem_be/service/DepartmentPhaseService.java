...
public class DepartmentPhaseService {
    ...
    // 1. Tạo departmentPhase với validation
    ...
    // 2. Tạo các giai đoạn cho department dựa trên InnovationPhase
    public List<DepartmentPhaseResponse> createPhasesForDepartmentFromInnovationPhase(String departmentId,
        ...
        // Tạo departmentPhase với cùng thời gian với InnovationPhase
    ...
    // 3. Lấy phases bằng department và round
    ...
    // 4. Lấy current phase của department hiện tại
    ...
    // 5. Cập nhật thời gian giai đoạn
    ...
    public DepartmentPhaseResponse updatePhase(String phaseId, UpdateDepartmentPhaseRequest request) {
        ...
        if (request.getPhaseType() != null && request.getPhaseType() != phase.getPhaseType()) {
            phase.setPhaseType(request.getPhaseType());
        }
        if (request.getStartDate() != null && request.getStartDate() != phase.getStartDate()) {
            phase.setStartDate(request.getStartDate());
        }
        if (request.getEndDate() != null && request.getEndDate() != phase.getEndDate()) {
            phase.setEndDate(request.getEndDate());
        }
        if (request.getDescription() != null && request.getDescription() != phase.getDescription()) {
            phase.setDescription(request.getDescription());
        }
        if (request.getIsActive() != null && request.getIsActive() != phase.getIsActive()) {
            phase.setIsActive(request.getIsActive());
        }
        if (request.getPhaseOrder() != null && request.getPhaseOrder() != phase.getPhaseOrder()) {
            phase.setPhaseOrder(request.getPhaseOrder());
        }
        if (request.getInnovationPhaseId() != null
                && request.getInnovationPhaseId() != phase.getInnovationPhase().getId()) {
            InnovationPhase innovationPhase = innovationPhaseRepository.findById(request.getInnovationPhaseId())
                    .orElseThrow(() -> new IdInvalidException(
                            "Không tìm thấy InnovationPhase với ID: " + request.getInnovationPhaseId()));
    ...
    /*
     * Validate time constraints for update
     */
    ...
    /*
     * Lấy department ID bằng phase ID
     */
    ...
    // 8. Tạo 3 phase cần thiết cho department
    public List<DepartmentPhaseResponse> createAllRequiredPhasesForDepartment(String departmentId, String roundId) {
        ...
        // Lấy tất cả innovation phases cho round này
        ...
        // Tạo phase cho mỗi innovation phase
    ...
    // 9. Lấy phases by department và round
    ...
    // 10. Lấy phase hiện tại của department trong round
    ...
    // 11. Tạo phase cần thiết cho department
    public DepartmentPhaseResponse createRequiredPhaseForDepartment(String departmentId, String roundId,
        ...
        // Validate time constraints cho department phase
    ...
    /*
     * Helper method để kiểm tra loại phase cần thiết
     */
